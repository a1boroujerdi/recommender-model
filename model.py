...
def cold_start() -> None:
    try:
        ...
        print(f"✅ Model trained. {len(product_list)} products loaded.")
...
def recommend() -> tuple:
    if rules is None or df is None:
        return jsonify({"error": "Model is still loading, try again soon"}), 503
    ...
    subset = rules[rules["antecedents"].apply(lambda x: matched in x)].sort_values(
    ...
    for _, row in subset.iterrows():
        for item in row["consequents"]:
            if item != matched and item not in recommended:
...
def spawn_trainer() -> None:
    threading.Thread(target=cold_start, daemon=True).start()


# ---------- run ----------
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=int(os.getenv("FLASK_RUN_PORT", 80)))
